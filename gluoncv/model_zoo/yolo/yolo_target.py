...
class YOLOV3PrefetchTargetGenerator(gluon.Block):
    ...
    def forward(self, img, xs, anchors, offsets, gt_boxes, gt_ids, gt_mixratio=None):
        ...
        xs : list of mxnet.np.ndArray
            List of feature maps.
        anchors : mxnet.np.ndarray
            YOLO3 anchors.
        offsets : mxnet.np.ndarray
            Pre-generated x and y offsets for YOLO3.
        gt_boxes : mxnet.np.ndarray
            Ground-truth boxes.
        gt_ids : mxnet.np.ndarray
            Ground-truth IDs.
        gt_mixratio : mxnet.np.ndarray, optional
            Mixup ratio from 0 to 1.

        Returns
        -------
        (tuple of) mxnet.np.ndarray
        ...
        all_anchors = mx.np.concatenate(*[a.reshape(-1, 2) for a in anchors], axis=0)
        assert isinstance(offsets, (list, tuple))
        all_offsets = mx.np.concatenate(*[o.reshape(-1, 2) for o in offsets], axis=0)
        ...
        with autograd.pause():
            ...
            center_targets = mx.np.zeros_like(shape_like)
            scale_targets = mx.np.zeros_like(center_targets)
            weights = mx.np.zeros_like(center_targets)
            objectness = mx.np.zeros_like(weights.split(axis=-1, num_outputs=2)[0])
            class_targets = mx.npx.one_hot(objectness.squeeze(axis=-1), depth=self._num_class)
            ...
            shift_gt_boxes = mx.np.concatenate(-0.5 * gtw, -0.5 * gth, 0.5 * gtw, 0.5 * gth, axis=-1)
            anchor_boxes = mx.np.concatenate(0 * all_anchors, all_anchors, axis=-1)  # zero center anchors
            shift_anchor_boxes = self.bbox2corner(anchor_boxes)
            ious = mx.npx.box_iou(shift_anchor_boxes, shift_gt_boxes).transpose((1, 0, 2))
    ...
    def _slice(self, x, num_anchors, num_offsets):
        ...
        return mx.np.concatenate(*ret, axis=1)
...
class YOLOV3DynamicTargetGeneratorSimple(gluon.HybridBlock):
    ...
    def forward(self, box_preds, gt_boxes):
        ...
        with autograd.pause():
            ...
            objness_t = mx.np.zeros_like(box_preds.as_nd_ndarray().slice_axis(axis=-1, begin=0, end=1)).as_np_ndarray()
            center_t = mx.np.zeros_like(box_preds.as_nd_ndarray().slice_axis(axis=-1, begin=0, end=2)).as_np_ndarray()
            scale_t = mx.np.zeros_like(box_preds.as_nd_ndarray().slice_axis(axis=-1, begin=0, end=2)).as_np_ndarray()
            weight_t = mx.np.zeros_like(box_preds.as_nd_ndarray().slice_axis(axis=-1, begin=0, end=2)).as_np_ndarray()
            class_t = mx.np.ones_like(objness_t.tile(reps=(self._num_class))) * -1
...
class YOLOV3TargetMerger(gluon.HybridBlock):
    ...
    def forward(self, box_preds, gt_boxes, obj_t, centers_t, scales_t, weights_t, clas_t):
        ...
        with autograd.pause():
            ...
            mask = obj[1] > 0
            objectness = mx.np.where(mask, obj[1], obj[0])
            mask2 = mask.tile(reps=(2,))
            center_targets = mx.np.where(mask2, centers[1], centers[0])
            scale_targets = mx.np.where(mask2, scales[1], scales[0])
            weights = mx.np.where(mask2, weights[1], weights[0])
            mask3 = mask.tile(reps=(self._num_class,))
            class_targets = mx.np.where(mask3, clas[1], clas[0])
            smooth_weight = 1. / self._num_class
            if self._label_smooth:
                smooth_weight = min(1. / self._num_class, 1. / 40)
                class_targets = mx.np.where(
                    class_targets > 0.5, class_targets - smooth_weight, class_targets)
                class_targets = mx.np.where(
                    (class_targets < -0.5) + (class_targets > 0.5),
                    class_targets, mx.np.ones_like(class_targets) * smooth_weight)
            class_mask = mask.tile(reps=(self._num_class,)) * (class_targets >= 0)
            return [mx.npx.stop_gradient(x) for x in [objectness, center_targets, scale_targets,
